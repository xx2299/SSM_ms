...
[v1,v2]=size(clay_avg);
clay_avg_temp=imresize(clay_avg,[v1/2,v2/2],'bilinear');
[w1,w2]=size(clay_avg_temp);
...
clay_avg_used=clay_avg_temp.';
clay_f=reshape(clay_avg_used,[w1*w2,1]);
clay_content=[clay_lat,clay_lon,clay_f];
[m1,n1]=size(clay_content);
[m2,n2]=size(SMAP1);
dif(m1,1)=0;
clay_new(m2,1)=0;
for j=1:m2
    count=1;
    for i=1:m1
        dif(count,1)=abs(clay_content(i,1)-SMAP1(j,1))+abs(clay_content(i,2)-SMAP1(j,2));
        count=count+1;
    end
    [~,idx]=min(dif);
    clay_new(j,1)=clay_content(idx,3);
    disp(j)
end
new=reshape(clay_new,[964,406]);
clay_content_new=new.';
% Change clay content has same surface coverage as SMAP
clay_content_knn=knn_mean(SMF_template,clay_content_new,1);
% Renormalize clay content to [0,1] (original clay content - [0.0102,0.98])
Flattened_mean_clay=clay_content_knn(:)';
MappedF_mean_clay=mapminmax(Flattened_mean_clay,0,1);
Mapped_mean_clay=reshape(MappedF_mean_clay,size(clay_content_knn));
% Remove regions with SSM less than 0.1
Mapped_mean_clay(SMAP_mean_SM<0.1)=nan;
...
[avg_mdif1_clay_Avg,avg_mdif1_clay_Std]=bin_value(avg_mdif1_80,Mapped_mean_clay);
[avg_mdif2_clay_Avg,avg_mdif2_clay_Std]=bin_value(avg_mdif2_80,Mapped_mean_clay);
[avg_mdif3_clay_Avg,avg_mdif3_clay_Std]=bin_value(avg_mdif3_80,Mapped_mean_clay);
[avg_mhpdif1_clay_Avg,avg_mhpdif1_clay_Std]=bin_value(avg_mhpdif1_80,Mapped_mean_clay);
[avg_mhpdif2_clay_Avg,avg_mhpdif2_clay_Std]=bin_value(avg_mhpdif2_80,Mapped_mean_clay);
[avg_mhpdif3_clay_Avg,avg_mhpdif3_clay_Std]=bin_value(avg_mhpdif3_80,Mapped_mean_clay);
[avg_hhpdif1_clay_Avg,avg_hhpdif1_clay_Std]=bin_value(avg_hhpdif1_80,Mapped_mean_clay);
[avg_hhpdif2_clay_Avg,avg_hhpdif2_clay_Std]=bin_value(avg_hhpdif2_80,Mapped_mean_clay);
[avg_hhpdif3_clay_Avg,avg_hhpdif3_clay_Std]=bin_value(avg_hhpdif3_80,Mapped_mean_clay);
% Plot the diagram (Figure S7)
plot1=subplot(3,3,1);
shadedErrorBar(x,avg_mdif1_clay_Avg,avg_mdif1_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'});
grid on
...
plot2=subplot(3,3,2);
shadedErrorBar(x,avg_mdif2_clay_Avg,avg_mdif2_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'YTick',[-1,-0.5,0,0.5,1])
set(gca,'FontSize',12)
...
plot3=subplot(3,3,3);
shadedErrorBar(x,avg_mdif3_clay_Avg,avg_mdif3_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'YTick',[-1,-0.5,0,0.5,1])
set(gca,'FontSize',12)
...
plot4=subplot(3,3,4);
shadedErrorBar(x,avg_mhpdif1_clay_Avg,avg_mhpdif1_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'FontSize',12)
xlabel('(d)','FontSize',15)
ylabel('H_{SEP_n} diff mean','FontSize',15,'FontWeight','bold')
plot5=subplot(3,3,5);
shadedErrorBar(x,avg_mhpdif2_clay_Avg,avg_mhpdif2_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'FontSize',12)
xlabel('(e)','FontSize',15)
plot6=subplot(3,3,6);
shadedErrorBar(x,avg_mhpdif3_clay_Avg,avg_mhpdif3_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'FontSize',12)
xlabel('(f)','FontSize',15)
plot7=subplot(3,3,7);
shadedErrorBar(x,avg_hhpdif1_clay_Avg,avg_hhpdif1_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'YTick',[-1,-0.5,0,0.5,1])
set(gca,'FontSize',12)
...
plot8=subplot(3,3,8);
shadedErrorBar(x,avg_hhpdif2_clay_Avg,avg_hhpdif2_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'YTick',[-1,-0.5,0,0.5,1])
set(gca,'FontSize',12)
xlabel('(h)','FontSize',15)
plot9=subplot(3,3,9);
shadedErrorBar(x,avg_hhpdif3_clay_Avg,avg_hhpdif3_clay_Std,'lineprops',{'-ro','MarkerFaceColor','r'})
grid on
set(gca,'YTick',[-1,-0.5,0,0.5,1])
set(gca,'FontSize',12)
...
h=text(0.383,-0.1,'Normalized soil clay content','FontSize',15,'FontWeight','bold');
